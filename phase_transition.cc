 ...
 namespace InitialConditions
 {
   ...
   public:
     InitialValues(const double           epsilon,
                   ...
                   const Extractors<dim> &ex,
                   const double           lambda_phi,
                   const double           lambda_psi,
   ...
   private:
       ...
       const double lambda_phi, lambda_psi, latent_heat;
...
double StokesProblem<dim>::compute_total_energy() const
{
  ...
  std::vector<Tensor<1, dim>> grad_psi(n_q);
  std::vector<double>          phi_val(n_q);
  std::vector<double>          psi_val(n_q);
  std::vector<double>          q_val(n_q);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    if (cell->is_locally_owned())
    {
      ...
      fe_values[extractors.phi_ch].get_function_values   (locally_relevant_solution, phi_val);
      fe_values[extractors.psi_ac].get_function_values   (locally_relevant_solution, psi_val);
      fe_values[extractors.aux_q  ].get_function_values  (locally_relevant_solution, q_val);
 ...
 {
   const std::string prefix = "tmp1036/";
